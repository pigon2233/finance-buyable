...
def run_backtest_for_stock(stock_code: str, days: int = 365) -> dict:
    ...
    try:
        ...
        print(f"  ⚠️ {stock_code} 錯誤: {e}", flush=True)
        return None

def main():
    days = int(sys.argv[1]) if len(sys.argv) > 1 else 365
    print(f"🚀 開始批量回測 {len(STOCKS)} 支股票（近 {days} 天）...", flush=True)
    print("=" * 60, flush=True)
    ...
    for i, stock in enumerate(STOCKS):
        print(f"[{i+1}/{len(STOCKS)}] 測試 {stock}...", end=" ", flush=True)
        result = run_backtest_for_stock(stock, days)
        
        if result:
            results.append(result)
            completed += 1
            print(f"✅ 報酬:{result['total_return']:+.2f}% 勝率:{result['win_rate']:.1f}%", flush=True)
        else:
            print("❌", flush=True)
    
    print("=" * 60, flush=True)
    print(f"✅ 完成 {completed}/{len(STOCKS)} 支股票回測", flush=True)
    print()
    ...
    if results:
        ...
        print(f"獲利股票: {len(profitable)} ({len(profitable)/len(results)*100:.1f}%)")
        print(f"虧損股票: {len(results)-len(profitable)} ({(len(results)-len(profitable))/len(results)*100:.1f}%)")
        print()
        print(f"平均報酬率: {sum(returns)/len(returns):+.2f}%")
        print(f"平均勝率: {sum(win_rates)/len(win_rates):.1f}%")
        print(f"平均最大回撤: {sum(max_dds)/len(max_dds):.2f}%")
        print(f"平均交易次數: {sum(trades)/len(trades):.1f}")
        print()
        ...
        for i, r in enumerate(sorted_by_return[:10]):
            print(f"  {i+1}. {r['stock']}: {r['total_return']:+.2f}% (勝率:{r['win_rate']:.1f}%)")
